...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let resetListenBtn = document.getElementById('reset-listen-btn'); // Changed to let for reassignment
    ...
    function setupListenVerifyDays() {
        ...
        if(listenFeedbackEl) {
            ...
            listenFeedbackEl.className = 'feedback-subsection'; // Reset feedback class
        }
        if(resetListenBtn) {
            resetListenBtn.classList.add('hidden'); // Hide reset button initially
            // Re-attach event listener to avoid duplicates if setup is called multiple times
            const newResetBtn = resetListenBtn.cloneNode(true);
            if (resetListenBtn.parentNode) {
    ...
    function handleDayListenClick(event) {
        ...
        const clickedDayItem = event.target.closest('.day-listen-item');
        if (!clickedDayItem || clickedDayItem.classList.contains('correct') || clickedDayItem.classList.contains('incorrect-clicked') || expectedDayIndex >= correctFullOrder.length) {
            return; // Ignore if already processed, or not a day item, or all done
        }
        playSound(clickSound);
        ...
        if (clickedDay === correctFullOrder[expectedDayIndex]) {
            ...
            clickedDayItem.style.cursor = 'default';
            ...
            if (expectedDayIndex === correctFullOrder.length) {
                ...
                Array.from(orderedDaysListEl.children).forEach(child => child.classList.add('disabled'));
            }
        } else {
            clickedDayItem.classList.add('incorrect-clicked');
            ...
            playSound(incorrectSound);
            setTimeout(() => clickedDayItem.classList.remove('incorrect-clicked'), 1000);
        }
    }
